...
impl Download {
    ...
    fn parse_download(value: &str) -> LottieDownload {
        if let Some(at_index) = value.find('@') {
            ...
            let end_index = value.rfind(".json").unwrap_or(value.len());
...
struct LottieDownload {
    name: String,
    url: String,
    builtin: Option<BuiltinLottieProps>,
}

impl LottieDownload {
    fn file_path(&self, directory: &Path) -> PathBuf {
        directory.join(&self.name).with_extension("json")
    }
