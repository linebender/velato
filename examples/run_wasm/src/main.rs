...
fn main() {
    // HACK: We rely heavily on compute shaders; which means we need WebGPU to
    // be supported However, that requires unstable APIs to be enabled,
    // which are not exposed through a feature
